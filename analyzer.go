...
func (wa *wslAnalyzer) flags() flag.FlagSet {
	...
	flags.IntVar(&wa.config.MaxAllowedStatementsAboveBlock, "max-allowed-statements-above-block", 1, "Max statements above 'for|range' block before requiring newline before block")
	flags.IntVar(&wa.config.MaxAllowedStatementsAboveIfBlock, "max-allowed-statements-above-if-block", 1, "Max statements above 'if' block before requiring newline before block")
